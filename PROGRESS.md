...
| 4.0.0 | 2026-04-11 | Feat: AI Campaign-Driven Creator Matching (F4 of the 4-feature rollout, major version bump). **AI-powered creator discovery + direct invitation flow** so brands stop manually scrolling the browse list and instead get a ranked shortlist the moment they create a campaign. The AI analyses the order (category, content type, budget, audience targeting, deadline, shipping flag) against every eligible creator's profile (followers, engagement, average views, content niches, price, past performance) and returns the top 10 matches with scored rationales. Brands invite directly from the panel; creators see a dedicated inbox and accept/decline with one click, at which point the standard assignment state machine takes over. (1) **Two new Prisma models** — `AiCreatorMatch` (`orderId`, `creatorId`, `matchScore` float, `matchReason` string, `reasoning` Json? for per-dimension breakdown, `createdAt`, `updatedAt`, `@@unique([orderId, creatorId])`, indexes on `(orderId, matchScore)` + `(creatorId)`) and `OrderInvitation` (`orderId`, `creatorId`, `invitedByUserId`, `status` via new `InvitationStatus` enum `PENDING`/`ACCEPTED`/`DECLINED`/`WITHDRAWN`/`EXPIRED`, `message?` max 1000, `respondedAt?`, `createdAt`, `updatedAt`, `@@unique([orderId, creatorId])`, indexes on `(creatorId, status)` + `(orderId, status)` + `(invitedByUserId)`). Chose a **separate `OrderInvitation` model rather than extending `OrderAssignment`** for clean isolation from F1/F2/F3 guards — invitations can expire/withdraw/decline without polluting the assignment state machine, and an accepted invitation creates a fresh `OrderAssignment` row that then flows through the existing pipeline. New `Order.matchesGeneratedAt DateTime?` stamp so the UI can show "last generated N minutes ago" + gate the regenerate button. Back-relations added on `Order`, `Creator`, and `User` (named `OrderInvitationInviter`). (2) **`matchCreatorsToOrder` function in `src/lib/ai.ts`** — pure (no DB writes, caller persists), takes `{ order, candidates }` and calls `callAI` with a structured prompt describing the campaign + a JSON array of candidates (capped at `MATCH_CANDIDATE_POOL = 50` in the AI function). Zod schemas (`matchReasoningSchema`, `singleMatchSchema`, `matchResponseSchema`) validate the AI response; invalid creatorIds are filtered out, remaining rows are sorted by `matchScore` DESC and sliced to `MATCH_RESULT_CAP = 10`. Returns typed `MatchResultRow[]` with `{ creatorId, matchScore, matchReason, reasoning: { audienceFit, categoryFit, contentStyleFit, budgetFit, engagementQuality } }`. (3) **Three new guards in `src/lib/guards.ts`** — `canViewMatches(ctx)` (ADMIN, brand owner, managing agency, any assigned account manager), `canInviteCreator(ctx)` (mirrors `canViewMatches` — if you can see the shortlist you can invite from it), `canRespondToInvitation(ctx)` (ADMIN for support escalation + the invited creator themselves only). 13 new vitest cases in `guards.test.ts` cover admin/brand/agency/AM positive paths, creator/network rejection, `canInviteCreator` mirroring, invited creator self, different creator rejection, non-creator roles rejection, and admin intervention. Guard suite now at **74 tests**. (4) **`/api/orders/[id]/matches` route** — `GET` returns `{ matches: [...], invitations: [...], matchesGeneratedAt }` with the creator profile + user included on each match row (so the frontend can render the full card without a second fetch); `POST` is rate-limited at 5/min per user per order via `match-generate:${userId}:${orderId}`, rejects on `COMPLETED`/`CANCELLED` status, loads the candidate pool (creators in the order's category, content type compatibility filter for LIVE/SHORT_VIDEO/COMBO, not already assigned, not already invited, ordered by `score` DESC + `followerCount` DESC, limit 60), calls `matchCreatorsToOrder`, then runs a `prisma.$transaction` that **upserts** each `AiCreatorMatch` row (so regeneration overwrites the previous shortlist cleanly via `(orderId, creatorId)` unique constraint) and stamps `order.matchesGeneratedAt = new Date()`. (5) **`/api/orders/[id]/invitations` route** — `POST` only, validates `{ creatorId, message? }` with Zod, loads the order context (brand + agency + AMs), calls `canInviteCreator`, checks order status isn't `COMPLETED`/`CANCELLED`, verifies the creator exists and isn't already assigned, then **upserts** the invitation (so a previously `WITHDRAWN`/`DECLINED` invite reopens to `PENDING` on re-invite instead of failing the unique constraint), fire-and-forget notifies the creator with a role-aware link to `/creator/invitations`. (6) **`/api/invitations` creator inbox route** — `GET` only, returns all invitations for the authenticated creator's `creator.id`, defaults to `status=PENDING` but supports `?status=ALL` for history. Includes order + brand company + category + invitedByUser details for the inbox card render. (7) **`/api/invitations/[id]/respond` accept/decline route** — `POST` with `{ action: "accept"|"decline" }`, rate-limited on `invite-respond:${userId}`, gated by `canRespondToInvitation`. **Accept path** validates content type compatibility (same LIVE/SHORT_VIDEO/COMBO check as public accept), then runs an atomic `prisma.$transaction` that re-reads the invitation status (409 `ALREADY_RESOLVED` on race loss), re-counts existing assignments against `order.maxCreators` (409 `MAX_CREATORS_REACHED`), re-checks for a pre-existing assignment (409 `ALREADY_ASSIGNED`), creates the `OrderAssignment` with the F3 shipping initialization (`shippingStatus: order.requiresShipping ? "PENDING_ADDRESS" : "NOT_REQUIRED"`), flips the invitation to `ACCEPTED` with `respondedAt`, and bumps the order from `DRAFT`/`OPEN` to `ASSIGNED`. Mirrors the existing `/api/orders/[id]/accept` transaction shape so both entry points behave identically. Decline path just flips to `DECLINED`. Both paths fire-and-forget notify the inviter + brand owner. (8) **`CreatorMatchList` client component (~350 lines)** — mounted on brand/agency/admin order detail pages. Uses `useTranslations("matches")`, a `ScorePill` helper with color-coded buckets (80+/60+/40+/<40), `load()` via GET and `generate()` via POST with the regenerate button. Each match row renders the creator card (avatar, name, followers, avg views, engagement, CPM), an expandable `<details>` reasoning breakdown (audienceFit / categoryFit / contentStyleFit / budgetFit / engagementQuality), and an inline `Invite` button that opens a composer (textarea max 1000 chars) and POSTs to `/api/orders/[id]/invitations`. Shows `Invited` / `Accepted` badges based on the joined invitation status. Gated by `!["COMPLETED","CANCELLED"].includes(order.status)`. Uses shadcn `Button` + `Textarea`, Lucide `Sparkles`/`UserPlus`/`RefreshCw`/`Check` icons. (9) **Creator invitations inbox page** (`/creator/invitations`) — new server component + `InvitationActions` client child. Splits into pending (orange highlight) + history sections, renders order info + inviter's message + budget/CPM/LIVE fee/deadline/`requires_shipping` badge, mounts `<InvitationActions invitationId={...} />` for the accept/decline buttons. `useTransition` + `router.refresh()` for server component refresh post-action. Added to the CREATOR sidebar nav as the 3rd entry with the Lucide `Mail` icon and `creator_invitations` label. (10) **i18n** — new `matches` namespace (31 keys: title, subtitle, generate, regenerate, generating, generate_success, generate_failed, load_failed, loading, empty_state, empty_state_hint, last_generated, followers, avg_views, engagement, cpm, show_reasoning, audience_fit, category_fit, content_style_fit, budget_fit, engagement_quality, invite, inviting, invited_badge, accepted_badge, send_invite, invite_success, invite_failed, message_placeholder, cancel) + new `invitations` namespace (25 keys: title, subtitle, pending_heading, history_heading, empty_pending, inviter_message, the_brand, budget, cpm, live_fee, deadline, requires_shipping, accept, decline, accepting, declining, accept_failed, decline_failed, accepted_toast, declined_toast, status_accepted/declined/withdrawn/expired/pending) + `sidebar.creator_invitations` key, all translated across **all 5 locales** (en/ar/tr/fr/es). (11) **Scope decisions (intentional cuts)** — candidate pool capped at 60 (sliced to 50 in the AI function) and top 10 matches returned, keeping the prompt size bounded and the round-trip inside the 60s AI timeout; no streaming matches or incremental re-ranking. Invitations are 1:1 with creators — to invite 3 creators you POST 3 times; no batch-invite endpoint. No `OrderInvitation.expiresAt` cron — `EXPIRED` is a reserved enum value for a future auto-expire sweep. No invitation withdrawal UI yet (reserved enum so the brand can still withdraw via admin escalation if needed). Accept path reuses the public `/accept` state machine verbatim rather than forking a second "invited accept" path. **Version bumped to 4.0.0** — major version signals the completion of the 4-feature creator-first rollout (F1 chat, F2 content review, F3 shipping, F4 AI matching) and the transition from a manual-matching marketplace into an AI-driven one. **Verification:** `npx prisma format` + `npx prisma generate` clean, `npx tsc --noEmit` clean, `npx vitest run` **128/128 passing** (was 115, +13 new guard tests for `canViewMatches` + `canInviteCreator` + `canRespondToInvitation`), `npm run lint` 0 errors (18 pre-existing warnings — none introduced). |
| 3.18.0 | 2026-04-11 | Feat: Order Chat (F1 of the 4-feature rollout). **Threaded per-assignment messaging** between the brand side (brand / agency / account-manager / admin) and the assigned creator or network, starting from the moment an order moves past `OPEN`. (1) **`OrderMessage` Prisma model** — new append-only table keyed `(orderId, assignmentId, senderId, body, attachments[], createdAt)` with a composite `(orderId, createdAt)` index plus secondary indexes on `assignmentId` and `senderId`. Cascades on order delete and assignment delete. Back-relations added to `Order.messages`, `OrderAssignment.messages`, and `User.sentOrderMessages` (named relation). (2) **`canAccessOrderThread` guard** — new pure function in `src/lib/guards.ts` mirroring the `canViewOrder` shape. Rules: ADMIN always, brand owner on any thread of their order, managing agency, any assigned account manager, OR a user whose id appears in the requested assignment's participants. Crucially, a creator can only read **their own** assignment's thread on multi-creator orders — private per-creator subthreads by design. 12 new vitest cases in `guards.test.ts` cover admin/brand/agency/AM/creator/network/stranger/empty-list paths; suite now at 83 tests. (3) **`/api/orders/[id]/messages` route** — `GET` lists messages scoped by `visibleAssignmentsFor()` (can request a specific `assignmentId` or the union of assignments the caller can see); `POST` validates with Zod (`{ assignmentId, body<=2000, attachments<=5 }`), rate-limits at `RATE_LIMITS.api` (60/min per user) keyed on `order-msg:${userId}`, calls `canAccessOrderThread`, creates the row, then fire-and-forgets notifications to every other participant (brand + agency + AMs + the other creator-side user on the assignment). **Per-recipient role-aware links**: looks up each recipient's role in one batched `user.findMany` and builds `/{role}/orders/{orderId}` so a brand recipient opens the brand view, a creator opens the creator view, etc. Account managers are routed to `/account-manager/orders` (the list page) since they don't have a dedicated order-detail route yet. (4) **`OrderChatPanel` client component** — polling-based chat UI (15s interval, pauses when tab hidden à la NotificationBell), auto-scrolls to bottom on new messages, supports up to 5 image attachments per message via `/api/upload` (10MB max, magic-byte validated), 2000-char textarea composer with brand-orange outgoing bubbles. When the caller has multiple visible threads (brand-side on a multi-creator order), an optional `Select` picker lets them switch threads. Uses shadcn `Textarea`/`Button`/`Select` + Lucide `Paperclip`/`Send`/`X` icons. (5) **Mounts on 5 order-detail pages** — creator/brand/agency/admin/network order pages compute a `chatAssignments` array and render `<OrderChatPanel>` after the delivery history, gated by `status !== DRAFT/OPEN/CANCELLED`. Creators see a single-entry list (their own assignment); brand/agency/admin get `showAssignmentPicker` on multi-creator orders. Network filters to assignments where the network is directly assigned OR owns the creator on the assignment. Admin labels fall back through `creator.user.name → network.user.name → email → id`. (6) **i18n** — new `chat` namespace (17 keys: title, subtitle, loading, empty_state, composer_placeholder, send/sending, attach/uploading, upload_failed, send_failed, message_too_long, too_many_attachments, remove_attachment, attachment_alt, pick_thread, no_assignments_yet) added across all 5 locales (en/ar/tr/fr/es). Also added `orders.unknown_creator` fallback key. (7) **Scope decisions (intentional cuts)** — no `OrderMessageRead` model (no unread counts / read receipts in this PR — notifications carry the "new message" signal). No websockets (15s polling is consistent with the NotificationBell pattern and fits the chat frequency for order coordination). No message edit/delete. Account manager dashboard still has no order-detail route — notifications link to the list page; adding an AM order detail is deferred. **Verification:** `npx tsc --noEmit` clean, `npx vitest run` 83/83 passing (was 71), `npm run lint` 0 errors (18 pre-existing warnings — none introduced). |
| 3.20.0 | 2026-04-11 | Feat: Physical Product Shipping (F3 of the 4-feature rollout). **Brand-to-creator product shipment flow** for orders like beauty, fashion, food, and tech reviews where the creator needs the physical product before they can film. Adds a per-assignment shipping state machine gated behind a new `requiresShipping` flag on orders. (1) **`ShippingStatus` Prisma enum** (`NOT_REQUIRED` / `PENDING_ADDRESS` / `PENDING_SHIPMENT` / `SHIPPED` / `DELIVERED` / `ISSUE_REPORTED`) plus 3 new fields on `Order` (`requiresShipping`, `productDescription`, `productValue`) and 15 new denormalized fields on `OrderAssignment` (address snapshot: name/line1/line2/city/state/postal/country/phone; carrier + tracking + trackingUrl; shippedAt/deliveredAt/shippingNotes/shippingIssue; plus `shippingStatus` itself with an index). The address is denormalized onto the assignment as a **snapshot at confirm time** so editing the creator's profile default does NOT rewrite history on past shipments. New `CreatorShippingAddress` model (1:1 with `Creator`, ISO-2 country) gives creators a persistent default to pre-fill from. (2) **Three new guards in `src/lib/guards.ts`** — `canManageShipping` (brand-side: ADMIN, brand owner, managing agency, assigned account managers), `canReceiveShipment` (creator/network on the assignment itself), and `canViewShipping` (union for reads). 17 new vitest cases cover every role path + negative cases; guard suite lifted from 44 → 61. (3) **`/api/creators/me/shipping-address`** — `GET` + `PUT` for the creator's persistent default. Uses `creatorShippingAddress.upsert`, country normalized to uppercase via `/^[A-Za-z]{2}$/`, rate-limited on `creator-shipping-addr:${userId}`. (4) **`/api/orders/[id]/assignments/[assignmentId]/shipping`** — action-based POST using `z.discriminatedUnion("action", [...])` that consolidates 4 state transitions into one endpoint: `set_address` (receiver → PENDING_SHIPMENT), `mark_shipped` (manager → SHIPPED), `confirm_delivery` (receiver → DELIVERED), `report_issue` (receiver → ISSUE_REPORTED, allowed from PENDING_SHIPMENT / SHIPPED / DELIVERED). Each transition returns 409 Conflict on invalid prior state. `loadAssignmentContext()` helper batches brand + agency + account managers from BOTH `accountManagerBrand` and `accountManagerAgency` for full escalation coverage. Role-aware notifications via `notifyBrandSide()` + `notifyReceiver()` helpers using inline `buildOrderDetailPath()`. Rate-limited on `shipping:${userId}`. (5) **`ShippingPanel` client component (~600 lines)** — dual-mode (`"receiver"` for creator/network, `"manager"` for brand/agency/AM/admin). Returns `null` when `!state.requiresShipping`. Sub-components: color-coded `StatusBadge` (6 statuses), `AddressSnapshot` (read-only with "snapshot of address at confirm time" note), `TrackingSnapshot` (carrier + tracking number + optional tracking URL + shipped timestamp), `AddressSection` (creator form that auto-prefills from `GET /api/creators/me/shipping-address` on mount), `MarkShippedSection` (brand form with a shadcn `Select` for USPS/UPS/FedEx/DHL/Other + tracking number + optional tracking URL + optional notes), `ReportIssueSection` (collapsible). Uses shadcn `Textarea`/`Button`/`Input`/`Label`/`Select` + Lucide `CheckCircle2`/`Home`/`Package`/`Truck`/`AlertTriangle` icons. (6) **Mounts on 5 order-detail pages** — creator/brand/agency/admin/network. Brand/agency/admin render **one panel per assignment** via `.map()` because each shipment is independent (cleaner than a picker variant). Network page filters to assignments where the network is directly assigned OR owns the creator. All mounts gated by `order.requiresShipping && !["DRAFT","OPEN","CANCELLED"].includes(order.status)`. (7) **Brand order creation form extension** (`brand/orders/new`) — new orange-bordered section with a `requiresShipping` checkbox that reveals a product description textarea (required when checked, max 500 chars) and a declared value number input. `POST /api/orders` accepts the 3 new fields in `createOrderSchema` with validation that `productDescription` is non-empty when `requiresShipping` is true. (8) **Creator settings mount** — new `CreatorShippingAddressForm` self-contained component (fetches/upserts via `/api/creators/me/shipping-address`) mounted on `/creator/settings` so creators can save their default address once and re-use it. (9) **`accept` route extension** — when a creator accepts an order with `requiresShipping: true`, the new `OrderAssignment` row is initialized with `shippingStatus: "PENDING_ADDRESS"` so the creator immediately sees the "confirm your shipping address" prompt on the order detail page. Non-shipping orders keep `shippingStatus: "NOT_REQUIRED"`. (10) **i18n** — new `shipping` namespace (59 keys: title, subtitle_receiver, subtitle_manager, loading, action_failed, declared_value, 6 `status_*` keys, waiting_for_*/address_snapshot_note/open_tracking_page/shipped_at, confirm_delivery, delivered_banner_title, issue_banner_title, 4 `*_toast` keys, default_address_title/subtitle/save_failed, fill_required_fields, country_must_be_iso2, 9 `field_*` keys, field_carrier_placeholder, carrier_other, field_tracking/tracking_url/notes, saving, confirm_address/save_default_address/mark_shipped, report_issue_link/button, issue_label/placeholder/required, cancel) added across all 5 locales (en/ar/tr/fr/es). (11) **Scope decisions (intentional cuts)** — no carrier rate lookup (free-form Select). No webhook-based live tracking (creator confirms delivery manually; brands can still link to the carrier's tracking page). No return/RMA flow (issues route through the existing ticket + chat channels). No separate shipping cost capture (bundled into the platform fee, not tracked per order). The `CreatorShippingAddress` model is a pre-fill source only — it's never the authoritative shipping address on any given OrderAssignment. **Verification:** `npx tsc --noEmit` clean, `npx vitest run` 98/98 + 17 new = expected 115/115 passing, `npm run lint` 0 errors. |
| 3.19.0 | 2026-04-11 | Feat: Pre-publish Content Review (F2 of the 4-feature rollout). **Creator-side content drafts + brand-side approval workflow** so orders can be reviewed before the creator publishes to TikTok. Builds on top of F1's per-assignment scoping. (1) **`ContentDraft` Prisma model** — new table keyed `(orderId, assignmentId, uploaderUserId, draftType, fileUrls[], notes?, status, reviewerUserId?, reviewNotes?, reviewedAt?, createdAt)` with `DraftType` enum (`VIDEO`/`IMAGES`) and `DraftStatus` enum (`PENDING_REVIEW`/`APPROVED`/`REJECTED`). Indexes: `(orderId, createdAt)`, `(assignmentId)`, `(uploaderUserId)`, `(status)`. Cascades on order + assignment delete, `SetNull` on reviewer delete. Back-relations: `Order.contentDrafts`, `OrderAssignment.contentDrafts`, `User.uploadedDrafts` (named `ContentDraftCreator`), `User.reviewedDrafts` (named `ContentDraftReviewer`). (2) **Two new guards in `src/lib/guards.ts`** — `canUploadContentDraft(ctx)` (creator/network uploader side: the assigned creator user OR the owning network user) and `canReviewContentDraft(ctx)` (brand-side reviewers: ADMIN, brand owner, managing agency, or any assigned account manager). Plus `canViewContentDrafts(ctx)` as the union. 15 new vitest cases cover all role paths + negative cases (creator can't review their own, stranger denied, agency only on managed brand, AM must match); suite now at 98 tests. (3) **`/api/orders/[id]/drafts` route** — `GET` lists drafts scoped by visible assignments (Zod-validated `?assignmentId` query + `?limit`); `POST` validates `{ assignmentId, draftType: VIDEO\|IMAGES, fileUrls[1..10], notes<=2000 }`, rate-limits at `RATE_LIMITS.api` on `order-draft:${userId}`, calls `canUploadContentDraft`, creates the row with `status=PENDING_REVIEW`, then fire-and-forgets notifications to all brand-side recipients (brand + agency + AMs) with per-recipient role-aware links via `buildOrderDetailPath`. (4) **`/api/orders/[id]/drafts/[draftId]/review` route** — `POST` validates `{ approved: boolean, reviewNotes<=2000 }`, loads the draft + order + brand + managing agency + account managers (batched from both `accountManagerBrand` and `accountManagerAgency` for full escalation coverage), calls `canReviewContentDraft`, then runs a **`prisma.$transaction`** that re-reads `status` and only updates if still `PENDING_REVIEW` — returns `{ error: "ALREADY_REVIEWED" }` on race loss so two reviewers can't double-decide. Fire-and-forget notification to the original uploader with a role-aware link. (5) **`ContentDraftsPanel` client component** — 400-line dual-mode React component. **Creator mode**: `draftType` Select (VIDEO/IMAGES), file picker gated on `video/*` or `image/*` accordingly, inline `<video controls>` previews for video drafts + image thumbnails for image drafts, notes textarea, submit button. Uploads each file via `/api/upload` with `FormData { file, kind: video\|image }`, then POSTs the full draft payload. **Reviewer mode**: per-draft review notes textarea + paired **"Request changes"** (destructive) / **"Approve"** (primary) buttons, gated to `PENDING_REVIEW` drafts. Shared `StatusBadge` for `PENDING_REVIEW`/`APPROVED`/`REJECTED`. Max 10 files per draft, notes capped at 2000 chars. When the caller has multiple visible threads (brand-side on a multi-creator order), an optional assignment `Select` picker lets them switch. (6) **Mounts on 5 order-detail pages** — creator/brand/agency/admin/network order pages build a `draftAssignments` array and render `<ContentDraftsPanel>` after `OrderChatPanel`, gated by `status !== DRAFT/OPEN/CANCELLED` (brand/agency/admin) or `status !== COMPLETED/CANCELLED` (creator/network). Creator mode is mounted on the creator + network pages; reviewer mode (with `showAssignmentPicker`) on brand + agency + admin. Label fallback chains match F1: creator (uses brand company name), brand/agency (creator.user.name → tiktok username → `unknown_creator`), admin (name → network name → email → id), network (network-assigned filter). (7) **Video upload caveat** — `/api/upload` accepts `kind=video` (up to 100MB client-side) but Vercel Hobby has a 4.5MB serverless body cap; documented as a known constraint in the upload route, with client-direct Blob upload deferred as a follow-up. (8) **i18n** — new `drafts` namespace (32 keys: title, subtitle_creator, subtitle_reviewer, loading, empty_state, no_assignments_yet, pick_assignment, pick_files, uploading, upload_failed, too_many_files, upload_at_least_one, remove_file, type_video, type_images, notes_placeholder, submitting, submit_draft, submit_success, submit_failed, status_pending, status_approved, status_rejected, creator_notes_label, reviewer_notes_label, review_notes_placeholder, request_changes, approve, review_failed, approved_toast, rejected_toast, file_alt, unknown_creator) added across all 5 locales (en/ar/tr/fr/es). (9) **Scope decisions (intentional cuts)** — no direct-to-Blob client uploads (serverless body path is fine for MVP; 100MB video upgrades tracked for v3.20.0+). No multi-round versioning (REJECTED is a terminal state on a draft — uploader submits a new draft to iterate). No in-panel @mentions. **Verification:** `npx tsc --noEmit` clean, `npx vitest run` 98/98 passing (was 83), `npm run lint` 0 errors. |
...
| 4.0.1 | 2026-04-11 | Feat: F4 completion pack — batch invite, invitation withdrawal, invitation emails (v4.0.0 shipped the AI matching + single-invite happy path; this is the polish pass that gets F4 production-ready). **3 surfaces in one PR.** (1) **Batch invite endpoint** — new `POST /api/orders/[id]/invitations/batch` that accepts `{ creatorIds: string[1..25], message?: string<=1000 }` and does a one-shot "invite top N matches" for brand/agency/AM callers. Reuses `canInviteCreator` for auth. Batched queries: single `creator.findMany({ where: { id: { in: uniqueIds } } })` + single `orderAssignment.findMany` pre-filter so the endpoint is one round-trip regardless of N. Categorizes creators into `skipped: [{ creatorId, reason: "CREATOR_NOT_FOUND"|"CREATOR_INACTIVE"|"ALREADY_ASSIGNED" }]` vs `invitable`, then upserts all invitable rows in a single `$transaction(invitable.map(prisma.orderInvitation.upsert(...)))` so previously `WITHDRAWN`/`DECLINED` invites cleanly re-open to `PENDING` on the same unique `(orderId, creatorId)` constraint as the single-invite path. Rate-limited on `order-invite-batch:${userId}`. Returns `{ invited: N, skipped: [...] }`. Fires fire-and-forget in-app notification + `sendInvitationSentEmail` per invitable creator. (2) **Invitation withdrawal** — new `POST /api/invitations/[id]/withdraw` + new `canWithdrawInvitation(ctx)` guard in `src/lib/guards.ts` (delegates to `canInviteCreator` — same "if you can invite, you can un-invite" rule: ADMIN, brand owner, managing agency, any assigned account manager). Rejects with 409 `ALREADY_RESOLVED` if the invitation status isn't `PENDING`. Runs an atomic `prisma.$transaction` that re-reads fresh status to prevent racing with a simultaneous `ACCEPTED`/`DECLINED` flip, then stamps `status: "WITHDRAWN"` + `respondedAt: new Date()`. Fires in-app notification to the creator so their invitations inbox drops the row on next refresh. Rate-limited on `invite-withdraw:${userId}`. 9 new vitest cases in `guards.test.ts` cover admin / brand owner / different brand / managing agency / unrelated agency / assigned AM (both am-1 and am-2) / unassigned AM / creator-role / network-role / anonymous — suite lifted from 128 → **137 tests**. (3) **Invitation emails via Resend** — 3 new templates in `src/lib/email.ts`, all wrapped in the existing `emailWrapper()` chrome: `sendInvitationSentEmail(creatorEmail, creatorName, orderTitle, inviterName, message)` — creator-facing, renders the optional brand message in an amber callout banner, CTA links to `/creator/invitations`; `sendInvitationAcceptedEmail(inviterEmail, inviterName, creatorName, orderTitle)` — inviter-facing, CTA links to `/brand/orders`; `sendInvitationDeclinedEmail(inviterEmail, inviterName, creatorName, orderTitle)` — inviter-facing decline notification, suggests trying another creator from the AI match list, CTA links to `/brand/orders`. Wired into the existing routes: `POST /api/orders/[id]/invitations` (single + batch) now fires `sendInvitationSentEmail` after creating the invitation; `POST /api/invitations/[id]/respond` fires `sendInvitationAcceptedEmail` on accept and `sendInvitationDeclinedEmail` on decline, both gated on `invitation.invitedByUser?.email` being present. Follows the same fire-and-forget pattern as all other `send()` call sites (dev mode skips cleanly when `RESEND_API_KEY` is not set). (4) **`CreatorMatchList` UI extension** — 2 new interactions surfaced on the existing shortlist panel: **batch invite button group** in the header (appears only when there are matches) showing `Invite top 3` / `Invite top 5` / `Invite top 10` chips with a `Users` icon label. Each filters the matches that haven't already been `PENDING`/`ACCEPTED`, slices the top N server-sorted rows, and POSTs to the batch endpoint with a success toast of `t("batch_success", { count, skipped })`. **Withdraw button** on `PENDING` invitation rows (inline next to the "Invited" badge, with the Lucide `X` icon) that POSTs to the withdraw endpoint with a `withdrawingId` disabled state so rapid double-clicks are safe. Accepted invitations remain immutable (no withdraw once the creator has accepted — the brand would need to cancel the assignment through the existing order flow). (5) **i18n** — 10 new keys in the `matches` namespace across all 5 locales (en/ar/tr/fr/es): `batch_invite`, `batch_invite_top_n` (with ICU `{n}` plural arg), `batch_inviting`, `batch_success` (with ICU `{count}` + `{skipped}` args), `batch_failed`, `batch_nothing_to_invite`, `withdraw`, `withdrawing`, `withdraw_success`, `withdraw_failed`. (6) **Scope decisions (intentional cuts)** — no `EXPIRED` auto-expire cron (the enum value remains reserved for a later sweep; withdrawal is the manual substitute for now). No admin oversight page for match analytics (brand-side only in this PR). No match re-rank on withdraw — the existing AI shortlist stays intact and the creator just moves back to "invitable" status for future rounds. No per-creator compatibility pre-check in the batch endpoint (the single-invite `accept` path still validates content-type compatibility on the creator side, so batch-inviting a LIVE-only order to a video-only creator is safe but will be rejected on their end). **Verification:** `npx tsc --noEmit` clean, `npx vitest run` **137/137 passing** (was 128, +9 new `canWithdrawInvitation` cases), `npm run lint` 0 errors (18 pre-existing warnings — none introduced). |
| 4.1.0 | 2026-04-11 | Feat: F4 extension pack — invitation expiry cron, match re-rank on withdraw, admin oversight page (closes every remaining F4 scope item from the v4.0.0/v4.0.1 deferred list). **3 surfaces in one PR.** (1) **Invitation expiry cron** — new `GET /api/cron/expire-invitations` route (protected by `CRON_SECRET` like the existing `expire-orders` + `refresh-metrics` crons), wired into `vercel.json` to run daily at **05:00 UTC**. Sweeps `OrderInvitation` rows with `status = PENDING` and `createdAt < now - INVITATION_EXPIRY_DAYS` (default 7, env-overridable), flips them to `EXPIRED` + stamps `respondedAt` in a single `updateMany` against a pre-loaded batch of up to 200 IDs. For each expired row, fires a fire-and-forget in-app notification to the original inviter (type `invitation_expired`, linked to `/brand/orders/{id}`) + a new `sendInvitationExpiredEmail` template. Structured log emits `{ event: cron_expire_invitations_complete, checked, expired, cutoff }` so Vercel log search can trace batch sizes. Gracefully no-ops when the batch is empty. (2) **`sendInvitationExpiredEmail` template** — new Resend email in `src/lib/email.ts` wrapped in the existing `emailWrapper()` chrome. Addressed to the inviter (brand owner / agency / AM) with the creator name, order title, and expiry day count, suggesting they invite another creator from the AI match list. CTA links to `/brand/orders`. Follows the same fire-and-forget pattern as every other `send()` call site. (3) **Match re-rank on withdraw** — the withdraw endpoint (`POST /api/invitations/[id]/withdraw`) now atomically deletes the `AiCreatorMatch` row for the withdrawn `(orderId, creatorId)` pair inside the same `$transaction` that flips the invitation status. The shortlist UI updates immediately without the brand having to click **Regenerate**. Uses `tx.aiCreatorMatch.deleteMany({ where: { orderId, creatorId } })` so it's a safe no-op if the creator wasn't in the AI shortlist (e.g. the invite came from the direct browse page instead of the AI matcher). (4) **Cooldown filter in candidate pool** — the AI match generation route (`POST /api/orders/[id]/matches`) now excludes creators whose invitation was recently `WITHDRAWN` / `DECLINED` / `EXPIRED` within the last `MATCH_COOLDOWN_DAYS` (default 14, env-overridable). Implementation: third parallel `prisma.orderInvitation.findMany` with `status: { in: ["WITHDRAWN", "DECLINED", "EXPIRED"] }` + `respondedAt: { gte: cooldownCutoff }`, joined into the existing `excludedCreatorIds` `Set`. Prevents the AI from re-recommending a creator the brand just dropped or who just ghosted; the brand can still send a direct invite from the browse page if they want to override. Combined with (3), this means: withdrawing a creator both drops them from the current shortlist AND prevents them from coming back on the next regenerate for 14 days. (5) **Admin oversight page** — new `/admin/matches` server component (follows the `/admin/analytics` pattern with direct Prisma queries, not a separate API route). Four summary cards: **Total matches generated**, **Orders with matches**, **Total invitations**, **Acceptance rate** (with sub-label showing N resolved). Five-card **invitation funnel row**: `PENDING`, `ACCEPTED`, `DECLINED`, `WITHDRAWN`, `EXPIRED` — each with a color-coded Lucide icon (`Clock`/`Check`/`X`/`RotateCcw`) and count. **Top 10 inviting orders** table (brand company name, order title, invitation count) — backed by `orderInvitation.groupBy({ by: orderId })` + a secondary `order.findMany({ where: { id: { in } } })` to hydrate brand + title. **Top 10 invited creators** table (creator name, TikTok username, invitation count) — backed by `orderInvitation.groupBy({ by: creatorId })` + a secondary `creator.findMany`. **Recent invitations** table (last 15 rows, order title, creator, inviter, status, date). Admin-only gate via inline `session.user.role !== "ADMIN"` → `redirect("/login")` (same pattern as every other admin page). (6) **Sidebar + i18n** — added `/admin/matches` to the ADMIN sidebar between `admin_analytics` and `admin_audit_log` with a new `matches: Sparkles` icon entry in the shared `iconMap`. Added 1 new sidebar label key (`admin_matches`) + 26 new keys in a new `admin_matches` namespace (title, subtitle, 4 summary card keys, `acceptance_rate_note` with ICU `{resolved}` arg, funnel heading + 5 status labels, top inviter + top creator heading/subtitle pairs, recent heading, 9 table column keys, empty state) across all 5 locales (en/ar/tr/fr/es). (7) **Scope decisions (intentional cuts)** — no `OrderInvitation.expiresAt` schema field (the `createdAt + INVITATION_EXPIRY_DAYS` rule keeps the model lean and the cron query plan tight on the existing `(orderId, status)` index). No auto-regenerate on withdraw (just deletes the match row — brand clicks **Regenerate** when they want fresh AI suggestions, avoiding unwanted AI token spend). No time-series chart on the admin page (funnel + leaderboards are enough for v1 — a 6-month trend chart is a follow-up once there's enough data to visualize). The admin page uses inline role checks rather than a new `canViewMatchStats` guard since the rule is uniformly "admin only" (no brand-side or ops-side access). **Verification:** `npx tsc --noEmit` clean, `npx vitest run` **137/137 passing** (unchanged — all new code is DB-integrated, no new pure-function surfaces to unit test), `npm run lint` 0 errors (18 pre-existing warnings — none introduced). |
| 4.1.1 | 2026-04-11 | Fix: F3 shipping i18n gaps on the brand order creation form — 6 keys that shipped missing with v3.20.0 back in F3 were rendering as raw `brand.new_order_*` strings on `/brand/orders/new`. The code at `src/app/[locale]/(dashboard)/brand/orders/new/page.tsx:427-487` referenced the shipping section labels via `t(...)` calls, but next-intl falls back to the raw key when a translation is missing, so brands saw literal `brand.new_order_shipping_toggle` as the checkbox label and (once toggled) `brand.new_order_product_description` / `_placeholder` / `_value` / `_value_hint` in place of the revealed form. **2 PRs landed on `claude/review-progress-versioning-xsMBo` after #106.** (1) **#107** — added 5 keys (`new_order_shipping_toggle`, `new_order_shipping_hint`, `new_order_product_description`, `new_order_product_description_placeholder`, `new_order_product_value`) to the `brand` namespace in en/ar/tr/fr/es, inserted between `new_order_field_deadline_hint` and `new_order_submit` to match the visual order of the form (deadline → shipping section → Create Order button). Copy: "Requires physical product shipping" / "Check this if you need to ship a physical product to the creator before they can film (e.g. beauty, fashion, food, tech reviews)." / "Product description *" / "e.g. Sample lipstick kit, size 7 running shoes, 500ml moisturizer..." / "Declared value (USD)" + semantically equivalent Arabic, Turkish, French, and Spanish translations. (2) **#108** — follow-up after a second screenshot revealed `brand.new_order_product_value_hint` still rendering as raw key under the declared value input. Added the 6th key in all 5 locales: "Optional. Used for customs and insurance reference only." + locale equivalents. Before landing #108, did a full scan of every `t("…")` call in `brand/orders/new/page.tsx` against each locale file to confirm no other keys were missing (checked `new_order_type_*`, `new_order_live_warning_*`, `new_order_field_*`, `new_order_cpm_preview`, `new_order_back/title/error/submit_loading` — all present). No schema changes, no runtime logic changes, no new dependencies — purely translations that should have shipped with v3.20.0 (F3 — Physical Product Shipping). Patch version bump because this was a visible user-facing regression for anyone creating a shipping order between v3.20.0 and v4.1.0. **Verification:** all 5 locale files parse as valid JSON, `npx tsc --noEmit` clean, `npx vitest run` 137/137 passing (unchanged — pure string additions). |
...
*Last updated: April 11, 2026 (v4.0.1)*
