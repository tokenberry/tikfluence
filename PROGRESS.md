...
### Future Refactors
- [ ] **Extract `CreatorCard` / `OrderCard` components**: list pages (`brand/browse`, `brand/orders`, agency/network equivalents) still render rows with inline JSX. Extracting reusable card components on top of the shadcn `Card` primitive would reduce duplication across ~6 pages.
...
2. **No separate `CreatorCard` / `OrderCard` files** - List rows are still inline JSX (shadcn `Card` primitive is available but not yet wrapped into domain-specific components — see "Future Refactors")
3. **Admin routes** - Split into separate route files instead of catch-all `admin/[...path]`
4. **No brands API route** - Brand profile managed through settings page
5. **Hosting** - Using Vercel instead of DigitalOcean Droplet
6. **Resend email integration** added (v0.2.0)
7. **shadcn/ui adopted late** - Originally built with plain Tailwind; full primitive migration landed in v3.2.0–v3.7.2 (PR #4a–#4h)
